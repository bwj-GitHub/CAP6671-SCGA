...
public class Server  extends Thread 
{
	...
	private java.util.Map<Integer, Integer>			resultsReceivedCounter; 
    ...
    Server()
	{
		...
		// Initailze results received counter
		receivedAllResults = false;
		resultsReceivedCounter = new HashMap<Integer, Integer>();
		for(Game game : games.allGames)
		{
			resultsReceivedCounter.put(game.getGameID(), 0);
		}
	...
	public void run()
	{
		...
		while (true)
		{
			try
			{
				...
				if (gamesScheduled == games.allGames.size())
				{
					log("No more games in games list, exiting main loop!");
					break;
				}
				...
				if (games.hasMoreGames())
				{
					System.out.println("Get next game");
    ...
    void shutDown()
	{
		System.out.println("Shutting down in 3 min");
		try
		{
				Thread.sleep(180000);
		}
    ...
    public synchronized void receiveGameResults(String address, Game game) 
	{
		try 
		{
			...
			int prevCount = resultsReceivedCounter.get(g.getGameID());
			resultsReceivedCounter.put(g.getGameID(), ++prevCount);
			
			receivedAllResults = true;
			for(Integer count : resultsReceivedCounter.values())
			{
